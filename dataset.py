...
class dataset(torch.utils.data.Dataset):
    def __init__(self, p_src, p_trg, case_sensitive=None, src_max_len=None, trg_max_len=None):
        p_list = [p_src]
        ...
        for line in itertools.izip_longest(*lines):
            line = map(lambda v: v.lower().strip() if not case_sensitive else v.strip(), line)
            if not any(line):
                continue
